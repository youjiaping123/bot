...
def record_auto(MIC_INDEX=0):
    ...
    QUIET_DB = 300           # 提高阈值，避免太容易触发
    delay_time = 2            # 增加到3秒，给更多说话时间
    
    FORMAT = pyaudio.paInt16
    CHANNELS = 1              # Windows下统一使用单声道
    ...
    while True:
        try:
            ...
            temp_time += 1  # 添加这行，记录时间帧
            
        except Exception as e:
            print(f"录音出错: {str(e)}")
            break
            
        if temp_time > 300:  # 增加超时时间到20秒
            END_TIME = temp_time
            print('\n超时，录音结束')
            break
